...
if ($pdo) {
    try {
        // Get products
        $stmt = $pdo->prepare("SELECT p.*, c.name as category_name FROM products p LEFT JOIN product_categories c ON p.category_id = c.id WHERE p.is_active = 1 ORDER BY p.name");
        $stmt->execute();
        $products = $stmt->fetchAll();
        
        // Get user's orders
        $stmt = $pdo->prepare("SELECT o.*, COUNT(oi.id) as item_count FROM orders o LEFT JOIN order_items oi ON o.id = oi.order_id WHERE o.user_id = ? GROUP BY o.id ORDER BY o.created_at DESC LIMIT 10");
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) as pending FROM orders WHERE user_id = ? AND status IN ('pending', 'confirmed', 'processing')");
        $stmt->execute([$user_id]);
        $stats['pending_orders'] = $stmt->fetch()['pending'];
        
        $stmt = $pdo->prepare("SELECT COALESCE(SUM(total), 0) as total FROM orders WHERE user_id = ? AND payment_status = 'paid'");
        $stmt->execute([$user_id]);
    ...
    <style>
        ...
        * {
        ...
        .product-card:hover {
            ...
            box-shadow: 0 5px 15px rgba(153,0,204,0.1);
        }
        ...
        .status-badge.processing { background: rgba(153,0,204,0.1); color: var(--primary); }
        .status-badge.shipped { background: rgba(0,102,102,0.1); color: var(--accent); }
        .status-badge.delivered { background: rgba(34,197,94,0.1); color: var(--success); }
        ...
        .payment-badge.paid { background: rgba(34,197,94,0.1); color: var(--success); }
        .payment-badge.unpaid { background: rgba(239,68,68,0.1); color: var(--danger); }
        .payment-badge.partial { background: rgba(245,158,11,0.1); color: var(--warning); }
...
<body>
    ...
    <aside class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="#products"><i class="fas fa-pills"></i> Products</a></li>
            <li><a href="#orders"><i class="fas fa-shopping-cart"></i> My Orders</a></li>
        </ul>
        
        <div class="menu-section">Finance</div>
        <ul class="sidebar-menu">
            <li><a href="#payments"><i class="fas fa-credit-card"></i> Payments</a></li>
            <li><a href="#invoices"><i class="fas fa-file-invoice"></i> Invoices</a></li>
        </ul>
        ...
        <div class="content">
            ...
            <div class="section-card" id="products">
                ...
                <div class="products-grid" id="productsGrid">
                    ...
                    <div class="product-card" data-category="<?php echo htmlspecialchars($product['category_name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                        <div class="product-category"><?php echo htmlspecialchars($product['category_name'] ?? 'General', ENT_QUOTES, 'UTF-8'); ?></div>
                        <div class="product-name"><?php echo htmlspecialchars($product['name'], ENT_QUOTES, 'UTF-8'); ?></div>
                        <div class="product-manufacturer"><?php echo htmlspecialchars($product['manufacturer'] ?? '', ENT_QUOTES, 'UTF-8'); ?></div>
                        <div class="product-price">₦<?php echo number_format($product['selling_price'], 2); ?></div>
                        <div class="product-actions">
                            <button class="btn btn-primary btn-sm" onclick="addToCart(<?php echo $product['id']; ?>, '<?php echo htmlspecialchars($product['name'], ENT_QUOTES, 'UTF-8'); ?>', <?php echo $product['selling_price']; ?>)">
                ...
                <table class="orders-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="payment-badge <?php echo $order['payment_status']; ?>"><?php echo ucfirst($order['payment_status']); ?></span></td>
